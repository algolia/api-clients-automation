...
using HttpScheme = Algolia.Search.Transport.HttpScheme;
...
server
  ...
  .RespondWith(
    Response.Create()
      .WithStatusCode(200)
      .WithBody(JsonConvert.SerializeObject(new BatchResponse() { }))
  );
...
{
  await searchIndex.SearchAsync<TestObject>(new Query());
}
...
Console.WriteLine(
  $"Saving {iterationsBatch * iterationsRecords} records by calling BatchAsync {iterationsBatch} times, with request payload containing {iterationsRecords} record of {JsonConvert.SerializeObject(CreateBody(0)).Length / 1024} ko.");

stopwatch.Restart();
for (var i = 0; i < iterationsBatch; i++)
{
  await searchIndex
    .BatchAsync(CreateRequests());
}
...
List<BatchOperation<object>> CreateRequests()
{
  var requests = new List<BatchOperation<object>>();

  for (var i = 0; i < iterationsRecords; i++)
  {
    var batchRequest = new BatchOperation<object>
    {

...
public static class SearchPlayground
{
  public static async Task Run(Configuration configuration)
  {
    ...
    search.Results.ForEach(result =>
    {
      ...
      {
        Console.WriteLine($"Record with Hits: ObjectID = {result.AsSearchResponse().Hits.First().ObjectID}, {result.AsSearchResponse().Hits.First().AdditionalProperties.Count}");
      }
    ...
    var configuredTaskAwaitable = await client
      .BrowseSynonymsAsync("test-csharp-new-client", new SearchSynonymsParams { Query = "",  Type = SynonymType.Onewaysynonym})
      .ConfigureAwait(false);

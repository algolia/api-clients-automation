...
module Algolia
  module AgentStudio
    ...
    class AlgoliaRecommendToolConfigOutput
      attr_accessor :name
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::AlgoliaRecommendToolConfigOutput` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::AlgoliaRecommendToolConfigOutput`. Please check the name to make sure it's valid. List of attributes: " +
      ...
      def hash
        [name, type, allowed_configs, predefined_recommend_parameters, description].hash
      end

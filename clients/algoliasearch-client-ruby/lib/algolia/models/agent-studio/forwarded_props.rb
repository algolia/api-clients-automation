    ...
    class ForwardedProps
      ...
      def self.types_mapping
        {
          ...
          :tool_approvals => :"Hash<String, Hash<String, Object>>",
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::ForwardedProps` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::ForwardedProps`. Please check the name to make sure it's valid. List of attributes: " +
        ...
        if attributes.key?(:tool_approvals)
          if (value = attributes[:tool_approvals]).is_a?(Hash)
      ...
      def hash
        [algolia, tool_approvals, configuration].hash
      end

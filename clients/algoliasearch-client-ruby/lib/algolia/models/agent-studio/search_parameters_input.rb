    ...
    class SearchParametersInput
      ...
      def self.types_mapping
        {
          ...
          :filters => :"String",
          :facet_filters => :"FacetFiltersInputUnion",
          :facets => :"FacetsUnion",
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::SearchParametersInput` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::SearchParametersInput`. Please check the name to make sure it's valid. List of attributes: " +

    ...
    class MessageResponseOutput
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::MessageResponseOutput` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::MessageResponseOutput`. Please check the name to make sure it's valid. List of attributes: " +
      ...
      def hash
        [id, conversation_id, role, parts, created_at, updated_at, model, input_tokens, output_tokens].hash
      end

    ...
    class UsageV4
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise ArgumentError, "The input argument (attributes) must be a hash in `Algolia::UsageV4` initialize method"
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::UsageV4`. Please check the name to make sure it's valid. List of attributes: " +
      ...
      def hash
        [prompt_tokens, completion_tokens, reasoning_tokens, total_tokens].hash
      end

 ...
 *   inner `waitForTask` call. When `null`, the default (100 retries) is used.
...
public suspend fun SearchClient.chunkedBatch(
  ...
  if (waitForTasks) {
    tasks.forEach { waitForTask(indexName = indexName, taskID = it.taskID, maxRetries = maxRetries) }
  }
  return tasks
}
 ...
 * @param requestOptions The requestOptions to send along with the query, they will be merged with
 *   the transporter requestOptions.
 * @param chunkedOptions Optional [ChunkedHelperOptions] controlling the polling budget on the inner
 *   `chunkedPush` call. When `null`, the default (100 retries) is used.
 ...
 *   the transporter requestOptions.
 * @param chunkedOptions Optional [ChunkedHelperOptions] controlling the polling budget on each
 *   inner `waitForTask` and the inner `chunkedPush` call. When `null`, the default (100 retries) is
 *   used.

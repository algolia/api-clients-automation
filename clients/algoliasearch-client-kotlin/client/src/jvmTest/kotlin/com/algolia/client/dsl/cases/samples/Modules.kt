...
/*
 * Application modules that each contribute one concern to a shared [QueryComposer]: locale
...
internal fun applyLocale(wrapper: QueryComposer, primary: String, secondary: String?) {
  wrapper.add {
    filters {
      orFacet {
...
internal fun applySearchableTitle(wrapper: QueryComposer) {
  wrapper.add {
...
internal fun applyBoost(wrapper: QueryComposer) {
  wrapper.add { optionalFilters { or { facet("isFeatured", true, score = 500) } } }
  wrapper.override { sumOrFiltersScores = true }
}

internal fun assemble(contributors: List<(QueryComposer) -> Unit>): SearchParamsObject {
  val wrapper = QueryComposer()
...
internal fun QueryComposer.setSuggestionType(request: SearchRequest) {
  request.filters[SUGGESTION_TYPE]?.let {
    add { filters { and { addFacet(SUGGESTION_TYPE, it) } } }
  }
}

internal fun QueryComposer.setPriority(request: SearchRequest) {
  request.filters[PRIORITY]?.toIntOrNull()?.let {
    add { filters { and { addFacet(PRIORITY, it) } } }
  }
}

internal fun QueryComposer.setPinned() {

//
// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.
//
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Text.Json;
using System.Text.Json.Serialization;
using Algolia.Search.Serializer;

namespace Algolia.Search.Models.AgentStudio;

/// <summary>
/// ToolResultPartOutput
/// </summary>
public partial class ToolResultPartOutput
{
  /// <summary>
  /// Initializes a new instance of the ToolResultPartOutput class.
  /// </summary>
  [JsonConstructor]
  public ToolResultPartOutput() { }

  /// <summary>
  /// Initializes a new instance of the ToolResultPartOutput class.
  /// </summary>
  /// <param name="toolCallId">toolCallId (required).</param>
  /// <param name="toolName">toolName (required).</param>
  /// <param name="output">output (required).</param>
  public ToolResultPartOutput(string toolCallId, string toolName, ToolResultOutput output)
  {
    ToolCallId = toolCallId ?? throw new ArgumentNullException(nameof(toolCallId));
    ToolName = toolName ?? throw new ArgumentNullException(nameof(toolName));
    Output = output ?? throw new ArgumentNullException(nameof(output));
  }

  /// <summary>
  /// Gets or Sets Type
  /// </summary>
  [JsonPropertyName("type")]
  public string Type { get; set; }

  /// <summary>
  /// Gets or Sets ToolCallId
  /// </summary>
  [JsonPropertyName("toolCallId")]
  public string ToolCallId { get; set; }

  /// <summary>
  /// Gets or Sets ToolName
  /// </summary>
  [JsonPropertyName("toolName")]
  public string ToolName { get; set; }

  /// <summary>
  /// Gets or Sets Output
  /// </summary>
  [JsonPropertyName("output")]
  public ToolResultOutput Output { get; set; }

  /// <summary>
  /// Gets or Sets ProviderOptions
  /// </summary>
  [JsonPropertyName("providerOptions")]
  public Dictionary<string, object> ProviderOptions { get; set; }

  /// <summary>
  /// Returns the string presentation of the object
  /// </summary>
  /// <returns>String presentation of the object</returns>
  public override string ToString()
  {
    StringBuilder sb = new StringBuilder();
    sb.Append("class ToolResultPartOutput {\n");
    sb.Append("  Type: ").Append(Type).Append("\n");
    sb.Append("  ToolCallId: ").Append(ToolCallId).Append("\n");
    sb.Append("  ToolName: ").Append(ToolName).Append("\n");
    sb.Append("  Output: ").Append(Output).Append("\n");
    sb.Append("  ProviderOptions: ").Append(ProviderOptions).Append("\n");
    sb.Append("}\n");
    return sb.ToString();
  }

  /// <summary>
  /// Returns the JSON string presentation of the object
  /// </summary>
  /// <returns>JSON string presentation of the object</returns>
  public virtual string ToJson()
  {
    return JsonSerializer.Serialize(this, JsonConfig.Options);
  }

  /// <summary>
  /// Returns true if objects are equal
  /// </summary>
  /// <param name="obj">Object to be compared</param>
  /// <returns>Boolean</returns>
  public override bool Equals(object obj)
  {
    if (obj is not ToolResultPartOutput input)
    {
      return false;
    }

    return (Type == input.Type || (Type != null && Type.Equals(input.Type)))
      && (
        ToolCallId == input.ToolCallId
        || (ToolCallId != null && ToolCallId.Equals(input.ToolCallId))
      )
      && (ToolName == input.ToolName || (ToolName != null && ToolName.Equals(input.ToolName)))
      && (Output == input.Output || (Output != null && Output.Equals(input.Output)))
      && (
        ProviderOptions == input.ProviderOptions
        || ProviderOptions != null
          && input.ProviderOptions != null
          && ProviderOptions.SequenceEqual(input.ProviderOptions)
      );
  }

  /// <summary>
  /// Gets the hash code
  /// </summary>
  /// <returns>Hash code</returns>
  public override int GetHashCode()
  {
    unchecked // Overflow is fine, just wrap
    {
      int hashCode = 41;
      if (Type != null)
      {
        hashCode = (hashCode * 59) + Type.GetHashCode();
      }
      if (ToolCallId != null)
      {
        hashCode = (hashCode * 59) + ToolCallId.GetHashCode();
      }
      if (ToolName != null)
      {
        hashCode = (hashCode * 59) + ToolName.GetHashCode();
      }
      if (Output != null)
      {
        hashCode = (hashCode * 59) + Output.GetHashCode();
      }
      if (ProviderOptions != null)
      {
        hashCode = (hashCode * 59) + ProviderOptions.GetHashCode();
      }
      return hashCode;
    }
  }
}

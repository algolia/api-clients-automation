...
func (r *ApiCompleteABTestRequest) UnmarshalJSON(b []byte) error {
	req := map[string]json.RawMessage{}
...
// ApiCompleteABTestRequest represents the request with all the parameters for the API call.
type ApiCompleteABTestRequest struct {
	id int32
}

// NewApiCompleteABTestRequest creates an instance of the ApiCompleteABTestRequest to be used for the API call.
func (c *APIClient) NewApiCompleteABTestRequest(id int32) ApiCompleteABTestRequest {
	return ApiCompleteABTestRequest{
		id: id,
	}
}

/*
CompleteABTest calls the API and returns the raw response from it.
...
You can't restart completed A/B tests.
	...
	Request can be constructed by NewApiCompleteABTestRequest with parameters below.
	  @param id int32 - Unique A/B test identifier.
	@param opts ...RequestOption - Optional parameters for the API call
	@return *http.Response - The raw response from the API
	@return []byte - The raw response body from the API
	@return error - An error if the API call fails
*/
func (c *APIClient) CompleteABTestWithHTTPInfo(r ApiCompleteABTestRequest, opts ...RequestOption) (*http.Response, []byte, error) {
	requestPath := "/3/abtests/{id}/complete"
	requestPath = strings.ReplaceAll(requestPath, "{id}", url.PathEscape(utils.ParameterToString(r.id)))
...
/*
CompleteABTest casts the HTTP response body to a defined struct.
...
Request can be constructed by NewApiCompleteABTestRequest with parameters below.
	...
	@return ABTestResponse
*/
func (c *APIClient) CompleteABTest(r ApiCompleteABTestRequest, opts ...RequestOption) (*ABTestResponse, error) {
	var returnValue *ABTestResponse

	res, resBody, err := c.CompleteABTestWithHTTPInfo(r, opts...)
	if err != nil {
		return returnValue, err
	}

package composition

// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.

import (
	"encoding/json"
	"fmt"
)

// ListCompositionsResponse struct for ListCompositionsResponse.
type ListCompositionsResponse struct {
	// All compositions in your Algolia application.
	Items []Composition `json:"items"`
	// Number of pages.
	NbPages int `json:"nbPages"`
	// Current page.
	Page int `json:"page"`
	// Number of items per page.
	HitsPerPage int `json:"hitsPerPage"`
	// Number of items.
	NbHits int `json:"nbHits"`
}

// NewListCompositionsResponse instantiates a new ListCompositionsResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed.
func NewListCompositionsResponse(items []Composition, nbPages int, page int, hitsPerPage int, nbHits int) *ListCompositionsResponse {
	this := &ListCompositionsResponse{}
	this.Items = items
	this.NbPages = nbPages
	this.Page = page
	this.HitsPerPage = hitsPerPage
	this.NbHits = nbHits

	return this
}

// NewEmptyListCompositionsResponse return a pointer to an empty ListCompositionsResponse object.
func NewEmptyListCompositionsResponse() *ListCompositionsResponse {
	return &ListCompositionsResponse{}
}

// GetItems returns the Items field value.
func (o *ListCompositionsResponse) GetItems() []Composition {
	if o == nil {
		var ret []Composition

		return ret
	}

	return o.Items
}

// GetItemsOk returns a tuple with the Items field value
// and a boolean to check if the value has been set.
func (o *ListCompositionsResponse) GetItemsOk() ([]Composition, bool) {
	if o == nil {
		return nil, false
	}

	return o.Items, true
}

// SetItems sets field value.
func (o *ListCompositionsResponse) SetItems(v []Composition) *ListCompositionsResponse {
	o.Items = v

	return o
}

// GetNbPages returns the NbPages field value.
func (o *ListCompositionsResponse) GetNbPages() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.NbPages
}

// GetNbPagesOk returns a tuple with the NbPages field value
// and a boolean to check if the value has been set.
func (o *ListCompositionsResponse) GetNbPagesOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.NbPages, true
}

// SetNbPages sets field value.
func (o *ListCompositionsResponse) SetNbPages(v int) *ListCompositionsResponse {
	o.NbPages = v

	return o
}

// GetPage returns the Page field value.
func (o *ListCompositionsResponse) GetPage() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.Page
}

// GetPageOk returns a tuple with the Page field value
// and a boolean to check if the value has been set.
func (o *ListCompositionsResponse) GetPageOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.Page, true
}

// SetPage sets field value.
func (o *ListCompositionsResponse) SetPage(v int) *ListCompositionsResponse {
	o.Page = v

	return o
}

// GetHitsPerPage returns the HitsPerPage field value.
func (o *ListCompositionsResponse) GetHitsPerPage() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.HitsPerPage
}

// GetHitsPerPageOk returns a tuple with the HitsPerPage field value
// and a boolean to check if the value has been set.
func (o *ListCompositionsResponse) GetHitsPerPageOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.HitsPerPage, true
}

// SetHitsPerPage sets field value.
func (o *ListCompositionsResponse) SetHitsPerPage(v int) *ListCompositionsResponse {
	o.HitsPerPage = v

	return o
}

// GetNbHits returns the NbHits field value.
func (o *ListCompositionsResponse) GetNbHits() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.NbHits
}

// GetNbHitsOk returns a tuple with the NbHits field value
// and a boolean to check if the value has been set.
func (o *ListCompositionsResponse) GetNbHitsOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.NbHits, true
}

// SetNbHits sets field value.
func (o *ListCompositionsResponse) SetNbHits(v int) *ListCompositionsResponse {
	o.NbHits = v

	return o
}

func (o ListCompositionsResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]any{}
	toSerialize["items"] = o.Items
	toSerialize["nbPages"] = o.NbPages
	toSerialize["page"] = o.Page
	toSerialize["hitsPerPage"] = o.HitsPerPage
	toSerialize["nbHits"] = o.NbHits

	serialized, err := json.Marshal(toSerialize)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal ListCompositionsResponse: %w", err)
	}

	return serialized, nil
}

func (o ListCompositionsResponse) String() string {
	out := ""
	out += fmt.Sprintf("  items=%v\n", o.Items)
	out += fmt.Sprintf("  nbPages=%v\n", o.NbPages)
	out += fmt.Sprintf("  page=%v\n", o.Page)
	out += fmt.Sprintf("  hitsPerPage=%v\n", o.HitsPerPage)
	out += fmt.Sprintf("  nbHits=%v\n", o.NbHits)

	return fmt.Sprintf("ListCompositionsResponse {\n%s}", out)
}

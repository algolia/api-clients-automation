package composition

// Code generated by OpenAPI Generator (https://openapi-generator.tech), manual changes will be lost - read more on https://github.com/algolia/api-clients-automation. DO NOT EDIT.

import (
	"encoding/json"
	"fmt"
)

// SearchCompositionRulesResponse struct for SearchCompositionRulesResponse.
type SearchCompositionRulesResponse struct {
	// Composition rules that matched the search criteria.
	Hits []CompositionRule `json:"hits"`
	// Number of composition rules that matched the search criteria.
	NbHits int `json:"nbHits"`
	// Current page.
	Page int `json:"page"`
	// Number of pages.
	NbPages int `json:"nbPages"`
}

// NewSearchCompositionRulesResponse instantiates a new SearchCompositionRulesResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed.
func NewSearchCompositionRulesResponse(hits []CompositionRule, nbHits int, page int, nbPages int) *SearchCompositionRulesResponse {
	this := &SearchCompositionRulesResponse{}
	this.Hits = hits
	this.NbHits = nbHits
	this.Page = page
	this.NbPages = nbPages

	return this
}

// NewEmptySearchCompositionRulesResponse return a pointer to an empty SearchCompositionRulesResponse object.
func NewEmptySearchCompositionRulesResponse() *SearchCompositionRulesResponse {
	return &SearchCompositionRulesResponse{}
}

// GetHits returns the Hits field value.
func (o *SearchCompositionRulesResponse) GetHits() []CompositionRule {
	if o == nil {
		var ret []CompositionRule

		return ret
	}

	return o.Hits
}

// GetHitsOk returns a tuple with the Hits field value
// and a boolean to check if the value has been set.
func (o *SearchCompositionRulesResponse) GetHitsOk() ([]CompositionRule, bool) {
	if o == nil {
		return nil, false
	}

	return o.Hits, true
}

// SetHits sets field value.
func (o *SearchCompositionRulesResponse) SetHits(v []CompositionRule) *SearchCompositionRulesResponse {
	o.Hits = v

	return o
}

// GetNbHits returns the NbHits field value.
func (o *SearchCompositionRulesResponse) GetNbHits() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.NbHits
}

// GetNbHitsOk returns a tuple with the NbHits field value
// and a boolean to check if the value has been set.
func (o *SearchCompositionRulesResponse) GetNbHitsOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.NbHits, true
}

// SetNbHits sets field value.
func (o *SearchCompositionRulesResponse) SetNbHits(v int) *SearchCompositionRulesResponse {
	o.NbHits = v

	return o
}

// GetPage returns the Page field value.
func (o *SearchCompositionRulesResponse) GetPage() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.Page
}

// GetPageOk returns a tuple with the Page field value
// and a boolean to check if the value has been set.
func (o *SearchCompositionRulesResponse) GetPageOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.Page, true
}

// SetPage sets field value.
func (o *SearchCompositionRulesResponse) SetPage(v int) *SearchCompositionRulesResponse {
	o.Page = v

	return o
}

// GetNbPages returns the NbPages field value.
func (o *SearchCompositionRulesResponse) GetNbPages() int {
	if o == nil {
		var ret int

		return ret
	}

	return o.NbPages
}

// GetNbPagesOk returns a tuple with the NbPages field value
// and a boolean to check if the value has been set.
func (o *SearchCompositionRulesResponse) GetNbPagesOk() (*int, bool) {
	if o == nil {
		return nil, false
	}

	return &o.NbPages, true
}

// SetNbPages sets field value.
func (o *SearchCompositionRulesResponse) SetNbPages(v int) *SearchCompositionRulesResponse {
	o.NbPages = v

	return o
}

func (o SearchCompositionRulesResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]any{}
	toSerialize["hits"] = o.Hits
	toSerialize["nbHits"] = o.NbHits
	toSerialize["page"] = o.Page
	toSerialize["nbPages"] = o.NbPages

	serialized, err := json.Marshal(toSerialize)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal SearchCompositionRulesResponse: %w", err)
	}

	return serialized, nil
}

func (o SearchCompositionRulesResponse) String() string {
	out := ""
	out += fmt.Sprintf("  hits=%v\n", o.Hits)
	out += fmt.Sprintf("  nbHits=%v\n", o.NbHits)
	out += fmt.Sprintf("  page=%v\n", o.Page)
	out += fmt.Sprintf("  nbPages=%v\n", o.NbPages)

	return fmt.Sprintf("SearchCompositionRulesResponse {\n%s}", out)
}

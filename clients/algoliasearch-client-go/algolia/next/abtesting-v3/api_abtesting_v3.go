...
//   - metrics - A/B test metrics involved in the test. Only these metrics will be considered when calculating results.
...
func (c *APIClient) AddABTests(
	...
	res, resBody, err := c.AddABTestsWithHTTPInfo(ctx, name, variants, metrics, endAt, configuration, opts...)
	if err != nil {
...
func (c *APIClient) AddABTestsWithHTTPInfo(
	...
	if len(metrics) == 0 {
		return nil, nil, reportError("Parameter `metrics` is required when calling `AddABTests`.")
	}
...
// GetTimeseriesOptions represents the optional params for the API call.
...
// NewGetTimeseriesOptions creates an instance of the GetTimeseriesOptions used to add optional parameters to GetTimeseriesWithOptions.
func NewGetTimeseriesOptions() *GetTimeseriesOptions {
	return &GetTimeseriesOptions{}
}

// WithStartDate Start date of the period to analyze, in `YYYY-MM-DD` format.
func (o *GetTimeseriesOptions) WithStartDate(startDate string) *GetTimeseriesOptions {
...
func (o *GetTimeseriesOptions) WithEndDate(endDate string) *GetTimeseriesOptions {
	o.EndDate = &endDate

	return o
}

// WithMetric List of metrics to retrieve. If not specified, all metrics are returned.
func (o *GetTimeseriesOptions) WithMetric(metric []MetricName) *GetTimeseriesOptions {
	o.Metric = &metric

	return o
}

// GetTimeseries Retrieves timeseries for an A/B test by its ID.
//
// Required API Key ACLs:
//   - analytics
//
...
//   - metric - List of metrics to retrieve. If not specified, all metrics are returned. (in optionalParams)
//   - opts - Optional parameters for the API call (e.g. WithHeaderParam, WithReadTimeout...)
func (c *APIClient) GetTimeseries(ctx context.Context, id int, optionalParams *GetTimeseriesOptions, opts ...RequestOption) (*Timeseries, error) {
	var returnValue *Timeseries

	res, resBody, err := c.GetTimeseriesWithHTTPInfo(ctx, id, optionalParams, opts...)
	if err != nil {
		return returnValue, err
	}
...
// GetTimeseriesWithHTTPInfo calls the API and returns the raw response from it.
...
//   - metric - List of metrics to retrieve. If not specified, all metrics are returned. (in optionalParams)
//   - opts - Optional parameters for the API call (e.g. WithHeaderParam, WithReadTimeout...)
func (c *APIClient) GetTimeseriesWithHTTPInfo(
	ctx context.Context,
	id int,
	optionalParams *GetTimeseriesOptions,
	opts ...RequestOption,
) (*http.Response, []byte, error) {
	...
	if optionalParams != nil && !utils.IsNilOrEmpty(optionalParams.Metric) {
		conf.queryParams.Set("metric", utils.QueryParameterToString(*optionalParams.Metric))
	}
...
// WithDirection Sort order for A/B tests by start date. Use 'asc' for ascending or 'desc' for descending. Active A/B tests are always listed first.
func (o *ListABTestsOptions) WithDirection(direction Direction) *ListABTestsOptions {
...
//   - direction - Sort order for A/B tests by start date. Use 'asc' for ascending or 'desc' for descending. Active A/B tests are always listed first.  (in optionalParams)
//   - opts - Optional parameters for the API call (e.g. WithHeaderParam, WithReadTimeout...)
...
func (c *APIClient) ScheduleABTest(
	...
	res, resBody, err := c.ScheduleABTestWithHTTPInfo(ctx, name, variants, metrics, scheduledAt, endAt, configuration, opts...)
	if err != nil {
		return returnValue, err
	}
...
func (c *APIClient) ScheduleABTestWithHTTPInfo(
	...
	if len(metrics) == 0 {
		return nil, nil, reportError("Parameter `metrics` is required when calling `ScheduleABTest`.")
	}

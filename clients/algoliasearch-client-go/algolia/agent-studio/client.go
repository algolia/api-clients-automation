...
// APIClient manages communication with the ragApi API v0.1.0
...
func NewClient(appID, apiKey string, region Region) (*APIClient, error) {
	return NewClientWithConfig(AgentStudioConfiguration{
...
func NewClientWithConfig(cfg AgentStudioConfiguration) (*APIClient, error) {
	...
	if cfg.ReadTimeout == 0 {
		cfg.ReadTimeout = 30000 * time.Millisecond
	}

	if cfg.ConnectTimeout == 0 {
		cfg.ConnectTimeout = 30000 * time.Millisecond
	}
...
func getDefaultHosts(r Region) []transport.StatefulHost {
	return []transport.StatefulHost{
		transport.NewStatefulHost("https", strings.ReplaceAll("agent-studio.{region}.algolia.com", "{region}", string(r)), call.IsReadWrite),
	}
}

func getUserAgent() string {
	return fmt.Sprintf("Algolia for Go (4.37.0); Go (%s); AgentStudio (4.37.0)", runtime.Version())
}

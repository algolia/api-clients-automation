...
class AnalyticsClient(
  ...
  /** Returns the static catalog of analytics fields, grouped by domain and usage (metrics, filters, groups,
    * distributions). No authentication is required. Use it to discover valid `(domain, kind)` pairs before building the
  ...
  /** Buckets one or more numeric fields into histograms and returns an object keyed by `histogram<Field>`, each mapping
    * a bin label to a count. `distributions` and `parameters` are required; `filters` is optional. Discover valid field

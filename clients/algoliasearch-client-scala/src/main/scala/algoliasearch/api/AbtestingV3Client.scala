...
class AbtestingV3Client(
  ...
  /** Completes a running A/B test by its ID. The A/B test's status becomes `expired` and its end date is set to the
    * completion time, mirroring what happens when an A/B test reaches its planned end date. The variant allocation is
    * removed from the search engine, like when stopping an A/B test. Unlike stopped A/B tests, completed A/B tests are
    * presented as finished rather than interrupted. Only running A/B tests can be completed. Completing an A/B test
    * that isn't running returns `409`. You can't restart completed A/B tests.
    *
    * Required API Key ACLs:
    *   - analytics
    *   - editSettings
    ...
    *   Unique A/B test identifier.
    */
  def completeABTest(id: Int, requestOptions: Option[RequestOptions] = None)(implicit
      ec: ExecutionContext
  ): Future[ABTestResponse] = Future {
    execute[ABTestResponse](completeABTestHttpRequest(id = id), requestOptions)
  }

  /** Variant of `completeABTest` that returns the full HTTP response: status code, headers, raw body and deserialized
    * data.
    *
    * Required API Key ACLs:
    *   - analytics
    ...
    *   Unique A/B test identifier.
    */
  def completeABTestWithHTTPInfo(id: Int, requestOptions: Option[RequestOptions] = None)(implicit
      ec: ExecutionContext
  ): Future[AlgoliaHttpResponse[ABTestResponse]] = Future {
    executeWithHttpInfo[ABTestResponse](completeABTestHttpRequest(id = id), requestOptions)
  }

  /** Validates the parameters and builds the request shared by `completeABTest` and `completeABTestWithHTTPInfo`.
    */
  private def completeABTestHttpRequest(id: Int): HttpRequest = {
    requireNotNull(id, "Parameter `id` is required when calling `completeABTest`.")

    HttpRequest
      .builder()
      .withMethod("POST")
      .withPath(s"/3/abtests/${escape(id)}/complete")
      .build()
  }

...
sealed trait CategoryPredictionBin

/** Confidence level of the category prediction.
  */
object CategoryPredictionBin {
  case object Certain extends CategoryPredictionBin {
    override def toString = "certain"
  }
  case object VeryHigh extends CategoryPredictionBin {
    override def toString = "very high"
  }
  case object High extends CategoryPredictionBin {
    override def toString = "high"
  }
  case object Low extends CategoryPredictionBin {
    override def toString = "low"
  }
  case object VeryLow extends CategoryPredictionBin {
    override def toString = "very low"
  }
  val values: Seq[CategoryPredictionBin] = Seq(Certain, VeryHigh, High, Low, VeryLow)

  def withName(name: String): CategoryPredictionBin = CategoryPredictionBin.values
    .find(_.toString == name)
    .getOrElse(throw new MappingException(s"Unknown CategoryPredictionBin value: $name"))
}

class CategoryPredictionBinSerializer
    extends CustomSerializer[CategoryPredictionBin](_ =>
      (
        {
          case JString(value) => CategoryPredictionBin.withName(value)
          case JNull          => null
        },
        { case value: CategoryPredictionBin =>

...
/** Partial search response returned when the `responseFields` parameter excludes fields like `hits`. Contains all
  * possible search response properties but none are required, so it acts as an unconditional fallback during oneOf
  * deserialization.
  *
  * @param abTestID
  ...
  */
case class SearchResponsePartial(
...
class SearchResponsePartialSerializer extends Serializer[SearchResponsePartial] {

  private val renamedFields = Map[String, String](
    "facets_stats" -> "facetsStats",
    "_automaticInsights" -> "automaticInsights"
  )
  override def deserialize(implicit format: Formats): PartialFunction[(TypeInfo, JValue), SearchResponsePartial] = {
    case (TypeInfo(clazz, _), json) if clazz == classOf[SearchResponsePartial] =>
      json match {
        case jobject: JObject =>
          ...
          val mf = manifest[SearchResponsePartial]
          Extraction.extract[SearchResponsePartial](renamedObject)(formats, mf)

        case _ => throw new IllegalArgumentException(s"Can't deserialize $json as SearchResponsePartial")
      }
  }

  override def serialize(implicit format: Formats): PartialFunction[Any, JValue] = {
    case value: SearchResponsePartial =>

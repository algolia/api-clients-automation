...
/** Whether NeuralSearch is active for the index. - `inactive`: keyword-only search is returned for live traffic. -
  ...
  */
object NeuralSearchMode {
  case object Inactive extends NeuralSearchMode {
    override def toString = "inactive"
  }
  case object Active extends NeuralSearchMode {
    override def toString = "active"
  }
  case object Preview extends NeuralSearchMode {
    override def toString = "preview"
  }
  val values: Seq[NeuralSearchMode] = Seq(Inactive, Active, Preview)

  def withName(name: String): NeuralSearchMode = NeuralSearchMode.values
    .find(_.toString == name)
    .getOrElse(throw new MappingException(s"Unknown NeuralSearchMode value: $name"))
}

class NeuralSearchModeSerializer
    extends CustomSerializer[NeuralSearchMode](_ =>
      (
        {
          case JString(value) => NeuralSearchMode.withName(value)
          case JNull          => null
        },
        { case value: NeuralSearchMode =>

...
export function createAgentStudioClient({
  ...
  return {
     ...
     *
     * Yields raw {@link ServerSentEvent} objects. Each event's `data` field contains a JSON-encoded `AgentCompletionResponse` string.
     *
    ...
    createAgentCompletionStreamRaw(
      ...
      if (!agentId) {
        throw new Error('Parameter `agentId` is required when calling `createAgentCompletionStreamRaw`.');
      }

      if (!compatibilityMode) {
        throw new Error('Parameter `compatibilityMode` is required when calling `createAgentCompletionStreamRaw`.');
      }

      if (!agentCompletionRequest) {
        throw new Error(
          'Parameter `agentCompletionRequest` is required when calling `createAgentCompletionStreamRaw`.',
        );
      }

      if (!agentCompletionRequest.messages) {
        throw new Error(
          'Parameter `agentCompletionRequest.messages` is required when calling `createAgentCompletionStreamRaw`.',
        );
      }
     ...
     *
     * Yields {@link StreamEvent} objects wrapping parsed `AgentCompletionResponse` payloads.
     *
    ...
    async *createAgentCompletionStream(
      { agentId, compatibilityMode, agentCompletionRequest, stream, cache, memory }: CreateAgentCompletionProps,
      requestOptions?: RequestOptions,
    ): AsyncGenerator<StreamEvent<AgentCompletionResponse>> {
      for await (const event of this.createAgentCompletionStreamRaw(
        { agentId, compatibilityMode, agentCompletionRequest, stream, cache, memory },
        requestOptions,
      )) {
        try {
          const data = JSON.parse(event.data) as AgentCompletionResponse;

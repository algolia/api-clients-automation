...
public class SearchClient extends ApiClient {
   ...
   * ChunkedHelperOptions#setMaxConcurrency(int)}. The {@code objects} Iterable is consumed on the
   * calling thread only.
   *
   * @summary Helper: Chunks the given `objects` list in subset of 1000 elements max in order to
   *     make it fit in `batch` requests.
  ...
  public <T> List<BatchResponse> chunkedBatch(
    ...
    while (true) {
      List<CompletableFuture<BatchResponse>> wave = buildWave(indexName, it, action, batchSize, maxConcurrency, helperRequestOptions);
      if (wave.isEmpty()) {
        break;
      }
      LaunderThrowable.await(CompletableFuture.allOf(wave.toArray(new CompletableFuture[0])));
   ...
   * resources may therefore surprise; use the synchronous variant, which consumes the Iterable on
   * the calling thread only.
   *
   * <p>Failures, including exceptions thrown by the {@code objects} Iterable, are reported through
   * the returned future. Only invalid arguments ({@code batchSize < 1}) throw directly.
   *
  ...
  public <T> CompletableFuture<List<BatchResponse>> chunkedBatchAsync(
    ...
    List<BatchResponse> responses = new ArrayList<>();
    Iterator<T> it;
    List<CompletableFuture<BatchResponse>> firstWave;
    try {
      it = objects.iterator();
      firstWave = buildWave(indexName, it, action, batchSize, maxConcurrency, helperRequestOptions);
    } catch (RuntimeException e) {
      CompletableFuture<List<BatchResponse>> failed = new CompletableFuture<>();
      failed.completeExceptionally(e);
      return failed;
    }
    return chunkedBatchAwaitWave(indexName, it, action, batchSize, maxConcurrency, helperRequestOptions, responses, firstWave);
  }

  /**
   * Reads up to {@code maxConcurrency} chunks of up to {@code batchSize} items from {@code it} and
   * sends one {@code batch} request per chunk. Never blocks; the iterator is consumed on the
   * calling thread.
   */
  private <T> List<CompletableFuture<BatchResponse>> buildWave(
    String indexName,
    Iterator<T> it,
    Action action,
    int batchSize,
  ...
  private <T> CompletableFuture<List<BatchResponse>> chunkedBatchAwaitWave(
    String indexName,
    Iterator<T> it,
    Action action,
    int batchSize,
    ...
    List<CompletableFuture<BatchResponse>> wave
  ) {
    if (wave.isEmpty()) {
    ...
    return CompletableFuture.allOf(wave.toArray(new CompletableFuture[0])).thenComposeAsync(ignored -> {
      ...
      List<CompletableFuture<BatchResponse>> nextWave = buildWave(indexName, it, action, batchSize, maxConcurrency, requestOptions);
      return chunkedBatchAwaitWave(indexName, it, action, batchSize, maxConcurrency, requestOptions, responses, nextWave);
    }, getExecutor());
  }

...
class AbtestingV3Client
{
     ...
     * @return ABTestResponse|array<string, mixed>
     */
    public function completeABTest($id, $requestOptions = [])
    {
        $response = $this->completeABTestWithHttpInfo($id, $requestOptions);
     ...
     *                                                       - $saveSettingsRequest['saveFeaturesSettings'] => (bool) Whether to also capture feature-specific settings for the variant
     *
    ...
    /**
     * Complete an A/B test (with HTTP info).
     *
     * Returns the response with HTTP metadata (status code, headers, body)
     * Completes a running A/B test by its ID.  The A/B test's status becomes `expired` and its end date is set to the completion time, mirroring what happens when an A/B test reaches its planned end date. The variant allocation is removed from the search engine, like when stopping an A/B test. Unlike stopped A/B tests, completed A/B tests are presented as finished rather than interrupted.  Only running A/B tests can be completed. Completing an A/B test that isn't running returns `409`.  You can't restart completed A/B tests.
     * Required API Key ACLs:
     ...
     * @return AlgoliaResponse
     */
    public function completeABTestWithHttpInfo($id, $requestOptions = [])
    {
        // verify the required parameter 'id' is set
        if (!isset($id)) {
            throw new \InvalidArgumentException(
                'Parameter `id` is required when calling `completeABTest`.'
            );
        }

        $resourcePath = '/3/abtests/{id}/complete';
        $queryParameters = [];
        $headers = [];
        $httpBody = null;

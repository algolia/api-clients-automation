...
| `replaceAllObjects`     | Given an `indexName` and an array of `objects`, replace all objects in this index using a temporary one                                                                                   | `operationIndex(), chunkedBatch()` | `none`                                                  | [PHP](https://github.com/algolia/api-clients-automation/blob/main/clients/algoliasearch-client-php/lib/Api/SearchClient.php#L2813)                                |
...
| `chunkedBatch`          | Creates chunks of `objects` in order to make them fit in multiple `batch` requests                                                                                                        | `none`                             | `none`                                                  | [PHP](https://github.com/algolia/api-clients-automation/blob/main/clients/algoliasearch-client-php/lib/Api/SearchClient.php#L2869)                                |
...
- won't have a destination index to `waitTask` on, if the `copy` can't succeed.

#### 2. chunk batch

Call the chunk batch for the given `objects`, call `waitTask` for the batching operation, which would create the `destination` index in case `copy` did not succeded.

#### 3. copy index again

Now that we are certain the destination index exist:
1. `waitTask` on the destination index, with the `taskID` returned from the first copy index invocation.
2. call `copy` again, if the first call succeeded, it will be a noop, otherwise, the destination index will be rebuilt.
3. `waitTask` on the destination index

#### 4. move destination index

Move the destination index to the source index, call `waitTask` on the destination index.

...
You might want to customize how the API client works, by providing extra information to your request, adding user-agents or use your own HTTP requester.

## Send extra information in your request with `requestOptions`

The `requestOptions` parameter allows you to merge extra information with the client transporter, such as `headers` or `query parameters`.

**Note: The `requestOptions` parameter is designed to override parameters that were previously sent.**
...
```js
// This request payload will contain the `extra-header` header, but override the `hitsPerPage` value.
await client.search(
  {
    requests: [
      {

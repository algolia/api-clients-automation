...
> The `waitForTask` method is only available in the `search` client context, as we don't awaitable tasks in other clients.
...
We provide a `waitForTask` helper method for you to easily fetch the task status.

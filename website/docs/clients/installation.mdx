...
// Single index Search
SearchParamsObject searchParams = new SearchParamsObject().setQuery("my aloglia ojbect");
SearchResponse<Map> response = client.searchSingleIndex("<YOUR_INDEX_NAME>", SearchParams.of(searchParams), Map.class);
System.out.println(response);

// Multi index search
SearchForHits query = new SearchForHits()
        .setIndexName("<YOUR_INDEX_NAME>")
        .setQuery("my aloglia ojbect");
SearchMethodParams searchMethodParams = new SearchMethodParams()
        .addRequests(SearchQuery.of(query));
SearchResponses<Object> responses = client.search(searchMethodParams);

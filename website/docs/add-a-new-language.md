...
HTTP 429 is not a host-failover case. Clients wait on the **same host** (`Retry-After` in whole seconds, or 1 second if the header is missing or invalid) up to `maxRateLimitRetries` (default 3; `0` fails on the first 429). 5xx, timeouts, and network errors still try other hosts.

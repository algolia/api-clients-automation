...
public class AbtestingV3ClientTests
{
  private readonly EchoHttpRequester _echo;
  private Exception _ex;

  public AbtestingV3ClientTests()
  {
  ...
  {
    var client = new AbtestingV3Client(new AbtestingV3Config("appId", "apiKey", "us"), _echo);
    await client.CustomPostAsync("1/test");
    EchoResponse result = _echo.LastResponse;
    {
      var regexp = new Regex(
        "^Algolia for Csharp \\(\\d+\\.\\d+\\.\\d+(-?.*)?\\)(; [a-zA-Z. ]+ (\\(\\d+((\\.\\d+)?\\.\\d+)?(-?.*)?\\))?)*(; AbtestingV3 (\\(\\d+\\.\\d+\\.\\d+(-?.*)?\\)))(; [a-zA-Z. ]+ (\\(\\d+((\\.\\d+)?\\.\\d+)?(-?.*)?\\))?)*$"
      );
  ...
  public async Task CommonApiTest1()
  {
    ...
    EchoResponse result = _echo.LastResponse;
    {
      var regexp = new Regex("^Algolia for Csharp \\(7.28.0\\).*");
      Assert.Matches(regexp, result.Headers["user-agent"]);
    }
  }
  ...
  public async Task ParametersTest0()
  {
    var client = new AbtestingV3Client(
      new AbtestingV3Config("my-app-id", "my-api-key", "us"),
      _echo
    );
  ...
  public async Task ParametersTest1()
  {
    ...
    {
      var client = new AbtestingV3Client(
        new AbtestingV3Config("my-app-id", "my-api-key", "not_a_region"),
        _echo
      );
    });
    Assert.Equal(

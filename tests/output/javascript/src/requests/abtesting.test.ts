...
describe('addABTests', () => {
  test('addABTests with minimal parameters', async () => {
    const req = (await client.addABTests({"endAt":"2022-12-31T00:00:00.000Z","name":"myABTest","variants":[{"index":"AB_TEST_1","trafficPercentage":30},{"index":"AB_TEST_2","trafficPercentage":50}]})) as unknown as EchoResponse;

    expect(req.path).toEqual('/2/abtests');
    expect(req.method).toEqual('POST');
    expect(req.data).toEqual({"endAt":"2022-12-31T00:00:00.000Z","name":"myABTest","variants":[{"index":"AB_TEST_1","trafficPercentage":30},{"index":"AB_TEST_2","trafficPercentage":50}]});
...
describe('estimateABTest', () => {
  test('estimate AB Test sample size', async () => {
    const req = (await client.estimateABTest({"configuration":{"emptySearch":{"exclude":true},"minimumDetectableEffect":{"size":0.03,"metric":"conversionRate"}},"variants":[{"index":"AB_TEST_1","trafficPercentage":50},{"index":"AB_TEST_2","trafficPercentage":50}]})) as unknown as EchoResponse;

    expect(req.path).toEqual('/2/abtests/estimate');
    expect(req.method).toEqual('POST');
    expect(req.data).toEqual({"configuration":{"emptySearch":{"exclude":true},"minimumDetectableEffect":{"size":0.03,"metric":"conversionRate"}},"variants":[{"index":"AB_TEST_1","trafficPercentage":50},{"index":"AB_TEST_2","trafficPercentage":50}]});
    expect(req.searchParams).toStrictEqual(undefined);
  });
...
describe('listABTests', () => {
  ...
  test('listABTests with parameters', async () => {
    const req = (await client.listABTests({"offset":0,"limit":21,"indexPrefix":"cts_e2e ab","indexSuffix":"t"})) as unknown as EchoResponse;

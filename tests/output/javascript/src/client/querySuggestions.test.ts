...
function createClient() {
  return algoliasearch(appId, apiKey).initQuerySuggestions({
...
describe('commonApi', () => {
  ...
  test('the user agent contains the latest version', async () => {
    ...
    expect(decodeURIComponent(result.algoliaAgent)).toMatch(/^Algolia for JavaScript \(5.58.0\).*/);
  }, 25000);
});

describe('noContent', () => {
  test('handles 204 No Content responses correctly', async () => {
    const client = algoliasearch('test-app-id', 'test-api-key').initQuerySuggestions({
      options: {
        hosts: [
          {
            url: 'localhost',
...
describe('init', () => {
  test('sets authMode', async () => {
    const qpClient = algoliasearch('foo', 'bar').initQuerySuggestions({
      options: { requester: nodeEchoRequester(), authMode: 'WithinQueryParameters' },
      region: 'us',
    });
    const headerClient = algoliasearch('foo', 'bar').initQuerySuggestions({
  ...
  test('sets authMode WithinBody', async () => {
    const bodyClient = algoliasearch('foo', 'bar').initQuerySuggestions({

  ...
  test('calls api with correct user agent', async () => {
      const client = createClient();
        ...
        const result = await client.customPost({"path":"1/test"}) as unknown as EchoResponse;
          ...
          expect(decodeURIComponent(result.algoliaAgent)).toMatch(/^Algolia for JavaScript \(\d+\.\d+\.\d+(-?.*)?\)(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*(; AbtestingV3 (\(\d+\.\d+\.\d+(-?.*)?\)))(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*$/);
  }, 25000);
  ...
  test('the user agent contains the latest version', async () => {
      const client = createClient();
        ...
        const result = await client.customPost({"path":"1/test"}) as unknown as EchoResponse;
          ...
          expect(decodeURIComponent(result.algoliaAgent)).toMatch(/^Algolia for JavaScript \(1.18.0\).*/);
  }, 25000);
...
describe('init', () => {
  test('sets authMode', async () => {
    const qpClient =  algoliasearch('foo', 'bar').initAbtestingV3({ options: { requester: nodeEchoRequester(), authMode: 'WithinQueryParameters' },  region:'us'  });;
    const headerClient =  algoliasearch('foo', 'bar').initAbtestingV3({ options: { requester: nodeEchoRequester(), authMode: 'WithinHeaders' },  region:'us'  });;

...
function createClient() {
  return agentStudioClient(appId, apiKey, { requester: nodeEchoRequester() });
}
...
describe('noContent', () => {
  test('handles 204 No Content responses correctly', async () => {
    const client = agentStudioClient('test-app-id', 'test-api-key', {
      hosts: [
        {
          url: 'localhost',
          port: 6692,
...
describe('parameters', () => {
  test('uses the correct host', async () => {
    const client = agentStudioClient('my-app-id', 'my-api-key', {
      requester: nodeEchoRequester(),
    });

    const result = (await client.customGet({ path: 'test' })) as unknown as EchoResponse;

    expect(result.host).toEqual('my-app-id-dsn.algolia.net');
  }, 25000);
});
...
describe('init', () => {
  test('sets authMode', async () => {
    const qpClient = agentStudioClient('foo', 'bar', {
      requester: nodeEchoRequester(),
      authMode: 'WithinQueryParameters',
    });
    const headerClient = agentStudioClient('foo', 'bar', { requester: nodeEchoRequester(), authMode: 'WithinHeaders' });
  ...
  test('sets authMode WithinBody', async () => {
    const bodyClient = agentStudioClient('foo', 'bar', { requester: nodeEchoRequester(), authMode: 'WithinBody' });

...
class AbtestingV3Test {
  ...
  fun `estimate AB Test sample size`() = runTest {
    client.runTest(
      ...
      intercept = {
        ...
        assertEquals(HttpMethod.parse("POST"), it.method)
        assertJsonBody("""{"configuration":{"minimumDetectableEffect":{"size":0.03,"metric":"conversionRate"}},"variants":[{"index":"AB_TEST_1","trafficPercentage":50},{"index":"AB_TEST_2","trafficPercentage":50}]}""", it.body)
      },
    )
  }
  ...
  fun `listABTests with parameters1`() = runTest {
    client.runTest(
      call = {
        listABTests(offset = 0, limit = 21, indexPrefix = "cts_e2e ab", indexSuffix = "t", direction = Direction.entries.first { it.value == "asc" })
      },
      intercept = {
        assertEquals("/3/abtests".toPathSegments(), it.url.pathSegments)
        assertEquals(HttpMethod.parse("GET"), it.method)
        assertQueryParams("""{"offset":"0","limit":"21","indexPrefix":"cts_e2e%20ab","indexSuffix":"t","direction":"asc"}""", it.url.encodedParameters)
        assertNoBody(it.body)
      },
    )
  }

...
class SearchTest {
  ...
  fun `indexExistsWithError`() = runTest {
    ...
    assertFails {
      ...
      .let { error ->
        assertError(
          error,
          "Client request\\(GET http://%localhost%:6681/1/indexes/indexExistsERROR/settings\\) invalid: 403 Forbidden. Text: \"\\{\"message\":\"Invalid API key\"\\}\""
            .replace(
              "%localhost%",
              if (System.getenv("CI") == "true") "localhost" else "host.docker.internal",
            ),
  ...
  fun `replaceAllObjects should cleanup on failure`() = runTest {
    ...
    assertFails {
      ...
      .let { error ->
        assertError(
          error,
          "Client request\\(POST http://%localhost%:6684/1/indexes/cts_e2e_replace_all_objects_too_big_kotlin_tmp_\\d+/batch\\) invalid: 400 Bad Request. Text: \"\\{\"message\":\"Record is too big\",\"status\":400\\}\""
            .replace(
              "%localhost%",
              if (System.getenv("CI") == "true") "localhost" else "host.docker.internal",
            ),
  ...
  fun `saveObjects should report errors`() = runTest {
    ...
    assertFails {
      ...
      .let { error ->
        assertError(
          error,
          "Client request\\(POST http://%localhost%:6680/1/indexes/cts_e2e_saveObjects_kotlin/batch\\) invalid: 403 Forbidden. Text: \"\\{\"message\":\"Invalid Application-ID or API key\",\"status\":403\\}\""
            .replace(
              "%localhost%",
              if (System.getenv("CI") == "true") "localhost" else "host.docker.internal",
            ),
  ...
  @Test
  fun `saveObjectsWithTransformation polls every task when waitForTasks is true`() = runTest {
    val client =
      SearchClient.withTransformation(
        appId = "test-app-id",
        apiKey = "test-api-key",
    ...
    client.runTest(
      call = {
        saveObjectsWithTransformation(
          indexName = "cts_e2e_chunked_push_wait_kotlin",
          objects =
            listOf(
              buildJsonObject {
                put(
  ...
  fun `wait for api key helper - add`() = runTest {
    ...
    client.runTest(
      ...
      response = {
        assertNotNull(it)
        JSONAssert.assertEquals(
          """{"value":"api-key-add-operation-test-kotlin","description":"my new api key","acl":["search","addObject"],"validity":300,"maxQueriesPerIPPerHour":100,"maxHitsPerQuery":20,"createdAt":1720094400}""",
          Json.encodeToString(Json.encodeToJsonElement(it)),
          JSONCompareMode.STRICT,
        )
      },
  ...
  fun `wait for api key - update`() = runTest {
    ...
    client.runTest(
      ...
      response = {
        assertNotNull(it)
        JSONAssert.assertEquals(
          """{"value":"api-key-update-operation-test-kotlin","description":"my updated api key","acl":["search","addObject","deleteObject"],"indexes":["Movies","Books"],"referers":["*google.com","*algolia.com"],"validity":305,"maxQueriesPerIPPerHour":95,"maxHitsPerQuery":20,"createdAt":1720094400}""",
          Json.encodeToString(Json.encodeToJsonElement(it)),
          JSONCompareMode.STRICT,
        )
      },

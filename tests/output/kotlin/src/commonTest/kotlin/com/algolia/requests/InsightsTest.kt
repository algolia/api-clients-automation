...
class InsightsTest {
  ...
  fun `deleteUserToken`() = runTest {
    client.runTest(
      call = {
        deleteUserToken(userToken = "test-user-1")
      },
      intercept = {
  ...
  fun `Many events type1`() = runTest {
    client.runTest(
      ...
      intercept = {
        assertEquals("/1/events".toPathSegments(), it.url.pathSegments)
        assertEquals(HttpMethod.parse("POST"), it.method)
        assertJsonBody("""{"events":[{"eventType":"conversion","eventName":"Product Purchased","index":"products","userToken":"user-123456","authenticatedUserToken":"user-123456","timestamp":1759708800000,"objectIDs":["9780545139700","9780439784542"],"queryID":"43b15df305339e827f0ac0bdc5ebcaa7"},{"eventType":"view","eventName":"Product Detail Page Viewed","index":"products","userToken":"user-123456","authenticatedUserToken":"user-123456","timestamp":1759708800000,"objectIDs":["9780545139700","9780439784542"]}]}""", it.body)
      },
    )
  }

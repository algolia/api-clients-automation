...
final class QuerySuggestionsClientClientTests: XCTestCase {
    ...
    func testParametersTest0() async throws {
        do {
            let configuration: QuerySuggestions.Configuration = try QuerySuggestions.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: ""))
    ...
    func testParametersTest1() async throws {
        do {
            let configuration: QuerySuggestions.Configuration = try QuerySuggestions.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: "not_a_region"))
            let transporter: Transporter = .init(configuration: configuration, requestBuilder: EchoRequestBuilder())
            let client = QuerySuggestionsClient(configuration: configuration, transporter: transporter)
    ...
    func testParametersTest2() async throws {
        let configuration: QuerySuggestions.Configuration = try QuerySuggestions.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: "us"))
        let transporter = Transporter(configuration: configuration, requestBuilder: EchoRequestBuilder())
        let client = QuerySuggestionsClient(configuration: configuration, transporter: transporter)
    }
}

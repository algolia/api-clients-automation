...
final class PersonalizationClientClientTests: XCTestCase {
    ...
    func testParametersTest0() async throws {
        do {
            let configuration: Personalization.Configuration = try Personalization.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: ""))
    ...
    func testParametersTest1() async throws {
        do {
            let configuration: Personalization.Configuration = try Personalization.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: "not_a_region"))
            let transporter: Transporter = .init(configuration: configuration, requestBuilder: EchoRequestBuilder())
            let client = PersonalizationClient(configuration: configuration, transporter: transporter)
    ...
    func testParametersTest2() async throws {
        let configuration: Personalization.Configuration = try Personalization.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: "us"))
        let transporter = Transporter(configuration: configuration, requestBuilder: EchoRequestBuilder())
        let client = PersonalizationClient(configuration: configuration, transporter: transporter)
    }
}

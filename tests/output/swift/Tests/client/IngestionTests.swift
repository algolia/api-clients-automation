...
final class IngestionClientClientTests: XCTestCase {
    ...
    func testParametersTest0() async throws {
        let configuration: Ingestion.Configuration = try Ingestion.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: "us"))
        let transporter = Transporter(configuration: configuration, requestBuilder: EchoRequestBuilder())
        let client = IngestionClient(configuration: configuration, transporter: transporter)
    ...
    func testParametersTest1() async throws {
        do {
            let configuration: Ingestion.Configuration = try Ingestion.Configuration(appId: "my-app-id", apiKey: "my-api-key", region: Region(rawValue: "not_a_region"))

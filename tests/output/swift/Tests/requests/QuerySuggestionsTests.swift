...
final class QuerySuggestionsClientRequestsTests: XCTestCase {
  ...
  func testCreateConfigTest0() async throws {
    ...
    let client: QuerySuggestionsClient = QuerySuggestionsClient(
      configuration: configuration, transporter: transporter)

    let response = try await client.createConfigWithHTTPInfo(
      querySuggestionsConfigurationWithIndex: QuerySuggestionsConfigurationWithIndex(
        indexName: "theIndexName",
        sourceIndices: [
          SourceIndex(
            indexName: "testIndex", facets: [Facet(attribute: "test")],
            generate: [["facetA", "facetB"], ["facetC"]])
        ], languages: Languages.arrayOfString(["french"]), exclude: ["test"]), requestOptions: nil)

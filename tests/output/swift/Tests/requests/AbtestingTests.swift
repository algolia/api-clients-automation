...
final class AbtestingClientRequestsTests: XCTestCase {
  ...
  func testAddABTestsTest0() async throws {
    ...
    let client: AbtestingClient = AbtestingClient(
    ...
    let response = try await client.addABTestsWithHTTPInfo(
      addABTestsRequest: AddABTestsRequest(
        ...
        ], endAt: "2022-12-31T00:00:00.000Z"), requestOptions: nil)
  ...
  func testListABTestsTest1() async throws {
    ...
    let response = try await client.listABTestsWithHTTPInfo(
      offset: 42, limit: 21, indexPrefix: "foo", indexSuffix: "bar", requestOptions: nil)
    let responseBodyData = try XCTUnwrap(response.bodyData)
    let echoResponse = try CodableHelper.jsonDecoder.decode(
      EchoResponse.self, from: responseBodyData)
    ...
    let comparableQueryItems = try XCTUnwrap(
      "{\"offset\":\"42\",\"limit\":\"21\",\"indexPrefix\":\"foo\",\"indexSuffix\":\"bar\"}".data(
        using: .utf8))
    let comparableQueryItemsMap = try CodableHelper.jsonDecoder.decode(
      StringMapObject.self, from: comparableQueryItems)

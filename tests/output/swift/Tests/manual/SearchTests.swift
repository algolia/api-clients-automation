...
final class SearchClientManualTests: XCTestCase {
  func testBuildFilters() throws {
    ...
    let helper = DisjunctiveFacetingHelper(query: SearchForHits(from: SearchSearchParamsObject(), indexName: "index_name"),
  ...
  func testMergeEmptyResponses() {
    ...
    do {
      ...
      XCTAssertEqual(error.localizedDescription, "Unexpected empty search responses list. At least one search responses might be present.")
    } catch {
  ...
  func testMergeDisjunctiveSingle() throws {
    ...
    var disjunctiveResponse: SearchResponse<Hit> = SearchResponse(processingTimeMS: 100, hits: [], query: "", params: "")
  ...
  func testMergeDisjunctiveDouble() throws {
    ...
    var firstDisjunctiveResponse: SearchResponse<Hit> = SearchResponse(processingTimeMS: 100, hits: [], query: "", params: "")
    ...
    var secondDisjunctiveResponse: SearchResponse<Hit> = SearchResponse(processingTimeMS: 100, hits: [], query: "", params: "")
  ...
  func testMergeFacetStats() throws {
    ...
    assertEqual(result.response.facetsStats?["price"], SearchFacetStats(min: 7, max: 103, avg: 55, sum: 3000),
                file: #filePath,
                line: #line)
    assertEqual(result.response.facetsStats?["target"], SearchFacetStats(min: 1, max: 10, avg: 5.5, sum: 43),
                file: #filePath,
                line: #line)
    assertEqual(result.response.facetsStats?["size"], SearchFacetStats(min: 20, max: 56, avg: 38, sum: 242),
                file: #filePath,
                line: #line)
    assertEqual(result.response.facetsStats?["note"], SearchFacetStats(min: 1, max: 5, avg: 3, sum: 37),
                file: #filePath,
                line: #line)
  }
  
  func assertEqual(_ lhs: SearchFacetStats?, _ rhs: SearchFacetStats?, file: StaticString = #filePath, line: UInt = #line) {

...
func TestSearchapi5(t *testing.T) {
	...
	client, err = search.NewClientWithConfig(cfg)
	require.NoError(t, err)
	res, err = client.CustomPost("1/test/gzip", search.NewCustomPostOptions().WithParameters(map[string]any{}).WithBody(map[string]any{"message": "this is a compressed body"}))
	require.NoError(t, err)
	rawBody, err := json.Marshal(res)
	require.NoError(t, err)
...
func TestSearchreplaceAllObjects0(t *testing.T) {
	...
	require.NoError(t, err)
	{
		res, err = client.ReplaceAllObjects("cts_e2e_replace_all_objects_go",
...
func TestSearchsaveObjects2(t *testing.T) {
	...
	require.NoError(t, err)
	{
		res, err = client.SaveObjects("playlists",

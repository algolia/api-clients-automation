...
class AgentStudioTest extends AnyFunSuite {
  ...
  def testClient(
      ...
      region: String = "us"
  ): (AgentStudioClient, EchoInterceptor) = {
  ...
  test("calls api with default read timeouts") {
    ...
    assert(echo.lastResponse.get.connectTimeout == 30000)
  ...
  test("calls api with correct user agent") {
    ...
    val regexp =
      """^Algolia for Scala \(\d+\.\d+\.\d+(-?.*)?\)(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*(; AgentStudio (\(\d+\.\d+\.\d+(-?.*)?\)))(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*$""".r
  ...
  test("the user agent contains the latest version") {
    ...
    val regexp = """^Algolia for Scala \(2.38.1\).*""".r
    val header = echo.lastResponse.get.headers("user-agent")
    assert(header.matches(regexp.regex), s"Expected $header to match the following regex: ${regexp.regex}")
  }

  test("uses the correct EU region") {

    val (client, echo) = testClient(appId = "my-app-id", apiKey = "my-api-key", region = "eu")
    Await.ready(
    ...
    assert(echo.lastResponse.get.host == "agent-studio.eu.algolia.com")
  }

  test("uses the correct US region") {
    ...
    assert(echo.lastResponse.get.host == "agent-studio.us.algolia.com")
  }

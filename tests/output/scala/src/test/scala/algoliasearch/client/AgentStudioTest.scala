...
class AgentStudioTest extends AnyFunSuite {
  ...
  def testClient(appId: String = "appId", apiKey: String = "apiKey"): (AgentStudioClient, EchoInterceptor) = {
  ...
  test("the user agent contains the latest version") {
    ...
    val regexp = """^Algolia for Scala \(2.40.1\).*""".r
    val header = echo.lastResponse.get.headers("user-agent")
    assert(header.matches(regexp.regex), s"Expected $header to match the following regex: ${regexp.regex}")
  }
  ...
  test("uses the correct host") {
    ...
    assert(echo.lastResponse.get.host == "my-app-id-dsn.algolia.net")
  }

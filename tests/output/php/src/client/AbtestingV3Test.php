...
/**
 * Client tests for AbtestingV3Client.
 *
 * @internal
 */
...
class AbtestingV3Test extends TestCase implements HttpClientInterface
{
    ...
    public function test0commonApi(): void
    {
        ...
        $this->assertTrue(
            (bool) preg_match(
                '/^Algolia for PHP \(\d+\.\d+\.\d+(-?.*)?\)(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*(; AbtestingV3 (\(\d+\.\d+\.\d+(-?.*)?\)))(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*$/',
    ...
    {
        $client = AbtestingV3Client::createWithConfig(AbtestingV3Config::create('test-app-id', 'test-api-key', 'us')->setFullHosts(['http://'.('true' == getenv('CI') ? 'localhost' : 'host.docker.internal').':6692']));
    ...
    {
        $client = AbtestingV3Client::createWithConfig(AbtestingV3Config::create('test-app-id', 'test-api-key', 'us')->setFullHosts(['http://'.('true' == getenv('CI') ? 'localhost' : 'host.docker.internal').':6683']));
     ...
     * @param mixed $region
     */
    private function createClient($appId, $apiKey, $region = 'us'): AbtestingV3Client
    {
        $config = AbtestingV3Config::create($appId, $apiKey, $region);
        $clusterHosts = AbtestingV3Client::getClusterHosts($config);

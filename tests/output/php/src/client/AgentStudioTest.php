...
/**
 * Client tests for AgentStudioClient.
 *
 * @internal
 */
...
class AgentStudioTest extends TestCase implements HttpClientInterface
{
    ...
    public function test0commonApi(): void
    {
        ...
        $this->assertTrue(
            (bool) preg_match(
                '/^Algolia for PHP \(\d+\.\d+\.\d+(-?.*)?\)(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*(; AgentStudio (\(\d+\.\d+\.\d+(-?.*)?\)))(; [a-zA-Z. ]+ (\(\d+((\.\d+)?\.\d+)?(-?.*)?\))?)*$/',
    ...
    {
        $client = AgentStudioClient::createWithConfig(AgentStudioConfig::create('test-app-id', 'test-api-key', 'us')->setFullHosts(['http://'.('true' == getenv('CI') ? 'localhost' : 'host.docker.internal').':6691']));
    ...
    #[TestDox('uses the correct EU region')]
    public function test0parameters(): void
    {
        ...
        $this->assertEquals(
            'agent-studio.eu.algolia.com',
            $this->recordedRequest['request']->getUri()->getHost()
        );
    }

    #[TestDox('uses the correct US region')]
    ...
    {
        $client = AgentStudioClient::createWithConfig(AgentStudioConfig::create('test-app-id', 'test-api-key', 'us')->setFullHosts(['http://'.('true' == getenv('CI') ? 'localhost' : 'host.docker.internal').':6683']));
     ...
     * @param mixed $region
     */
    private function createClient($appId, $apiKey, $region = 'us'): AgentStudioClient
    {
        $config = AgentStudioConfig::create($appId, $apiKey, $region);
        $clusterHosts = AgentStudioClient::getClusterHosts($config);
